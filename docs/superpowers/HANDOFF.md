...
> **Data:** 2026-06-16 · **Branch:** `main` · **HEAD:** `c3f9a43` (più eventuali commit doc successivi — fai `git log`) · **Stato:** Piani 1-6 + **7a/7b/7c + 8a/8b** completi e mergiati (engine + persistenza + Provider Layer AI + StructuredOutputPort + AI Master pipeline + Canon Ledger L1.5 + **Reflection/L2/Salienza**), **202 test verdi**, typecheck pulito, tree pulito.
>
...
L'engine (Piani 1-5), la **Persistenza (6)**, il **Provider Layer AI (7a)**, lo **StructuredOutputPort (7b)**, l'**AI Master pipeline (7c)**, il **Canon Ledger L1.5 (8a)** e **Reflection + L2 + Salienza (8b)** sono **finiti e mergiati** in `main`. Il Piano 7 è completo; il **Piano 8 è splittato in 8a/8b/8c** (come il 7) e **8a + 8b sono fatti**. Il prossimo passo è **scrivere ed eseguire il Piano 8c — Context Assembler con budget di token (spec §6.2)**: un allocatore che legge L1 (GameState/engine) + L1.5 (8a `canon_facts`) + L2 (8b `summaries`), applica la **recency a tempo di lettura** (decadimento sul `created_at` che 8b timbra via la porta `Clock`) per il ranking dei ricordi, e produce il contesto entro un budget con degrado controllato (mai tagliare L1/L1.5); **rimpiazza `assembleContextStub`** di 7c ed è **iniettato** in `runMasterTurn` (perché `ai` non può importare `memory`). Valuta se infilare in 8c (o in un piano dedicato) i `Command`/`Event` engine mancanti per gli strumenti rimandati di 7c (`request_check`/`apply_effect`/`advance_quest`) e la **FSM di fase** (spec §5.5). Tutti i piani sono in `docs/superpowers/plans/`.
...
- **Piani (tutti in `docs/superpowers/plans/`):**
  ...
  - **Piano 8c → prossimo da scrivere** (Context Assembler con budget di token; vedi §0/§7/§8).
...
Monorepo **pnpm workspaces** (`pnpm-workspace.yaml` globba `packages/*` e `app/*`). Esistono `packages/engine` (dettagliato qui sotto), più `packages/shared` e `packages/memory` (Piano 6 + 8a + 8b — vedi §3-bis) e `packages/ai` (Piani 7a/7b/7c — vedi §3-ter). TS strict (`tsconfig.base.json`): `strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `noImplicitOverride`, `verbatimModuleSyntax`. Test: Vitest (config root `vitest.config.ts`, include `packages/**/*.test.ts`). **202 test verdi totali** (98 engine + 55 `shared`/`memory` + 49 `ai`).
...
- **Verifica (dalla root):** `pnpm test` (Vitest, atteso **202 verdi**), `pnpm typecheck` (→ `pnpm -r typecheck` → `tsc --noEmit` su engine/shared/memory/ai), `pnpm -C packages/<pkg> typecheck` per il singolo pacchetto.
- I subagent creano file con lo strumento Write (NON `New-Item -Force`, che tronca).

---

## 7. Piano 7 (7a/7b/7c) e Piano 8a/8b — COMPLETI. PROSSIMO PASSO: Piano 8c

> **Piano 8a — Canon Ledger (L1.5) ✅ FATTO e mergiato** — vedi `docs/superpowers/plans/2026-06-16-loomn-fase1-piano8a-canon-ledger.md` e §3-bis. `canon_facts` (proiezione SQLite) + `createCanonLedger` (record/active/all/retract/supersede). **Piano 8b — Reflection + L2 + Salienza ✅ FATTO e mergiato** — vedi `docs/superpowers/plans/2026-06-16-loomn-fase1-piano8b-reflection-l2.md` e §3-bis: tabella `summaries` (L2) + `createSummaryStore`; `scoreSalience` (importanza×ricorrenza, output [0.1,1]) + porta `Clock`; colonna `salience` su `canon_facts` (migrazione 0003 ALTER); `runReflection` (porte `FactExtractor`/`Summarizer`/`Clock` **iniettate**, asincrona/fuori dal turno, testato coi doppi). Migrazioni ancora **scritte a mano** (drizzle-kit rimandato, §3-bis). Il **Piano 8 è splittato in 8a (fatto) / 8b (fatto) / 8c**; il prossimo è **8c** (Context Assembler, vedi §0/§8). Il follow-up PK di 8a (`record`/`supersede` lanciano su `id` duplicato) è **gestito da 8b** con id deterministici per range (precondizione: una Reflection per range).
...
- **Piano 8c — Context Assembler** ← *prossimo* (budget di token §6.2; legge L1+L1.5+L2, applica la recency a tempo di lettura sul `created_at` di 8b; rimpiazza `assembleContextStub`; iniettato in `runMasterTurn`)
- **Piano 9 — Shell Electron** (main/preload/renderer, sicurezza contextIsolation/sandbox/safeStorage, IPC tipizzato, **Clock** reale (porta definita in 8b) per i meta degli eventi e il `created_at` di L2; impl LLM-backed di `FactExtractor`/`Summarizer` per la Reflection (usano lo `StructuredOutputPort` di 7b — composizione `ai`+`memory` solo qui nell'app); wiring di EventStore+CanonLedger+SummaryStore su connessione condivisa)
...
1. Leggi questo file + lo spec (per il Piano 8c contano **§6.2** Context Assembler con budget di token e **§6** memoria a strati) + (se utile) i Piani 7c (`assembleContextStub` + `runMasterTurn`), 8a (`createCanonLedger`) e 8b (`createSummaryStore`, `scoreSalience`/`Clock`, `runReflection`).
2. `git status` (atteso pulito, su `main`) e `pnpm test` (atteso **202 verdi**), `pnpm typecheck` pulito.
3. Quando l'utente dice "scrivi il Piano 8c": usa `writing-plans`, applica le house rules (§5), parti dalla sintesi in §0/§8. Esiste già `@loomn/memory` con EventStore SQLite + snapshot (6), **Canon Ledger L1.5** (8a, `createCanonLedger`) e **L2/Reflection/Salienza** (8b: `createSummaryStore`, `scoreSalience`, porta `Clock`, `runReflection`; `canon_facts` ha `salience`, `summaries` ha `salience`+`created_at`); il turno del Master (7c) ha `assembleContextStub`. Piano 8c = **Context Assembler** con budget di token (spec §6.2): legge L1 (GameState/engine) + L1.5 (8a) + L2 (8b), applica la **recency a tempo di lettura** (decadimento sul `created_at` timbrato da 8b via Clock) per il ranking, e produce il contesto entro budget con **degrado controllato** (mai tagliare L1/L1.5); **rimpiazza `assembleContextStub`** ed è **iniettato** in `runMasterTurn` (l'iniezione/wiring è all'app o via parametro, perché `ai` non importa `memory`). Valuta se includere i `Command`/`Event` engine per gli strumenti rimandati di 7c (`request_check`/`apply_effect`/`advance_quest`) e la FSM di fase (spec §5.5).
