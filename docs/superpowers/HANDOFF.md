...
> **Data:** 2026-06-16 · **Branch:** `main` · **HEAD:** `cbac493` (più eventuali commit doc successivi — fai `git log`) · **Stato:** Piani 1-6 + **7a/7b/7c + 8a/8b/8c + 9a** completi e mergiati (engine + persistenza + Provider Layer AI + StructuredOutputPort + AI Master pipeline + Canon Ledger L1.5 + Reflection/L2/Salienza + Context Assembler + **Shell Electron `app/desktop`**), **221 test verdi**, typecheck pulito (incluso `app/desktop` via `vue-tsc`), tree pulito.
>
...
L'engine (Piani 1-5), la **Persistenza (6)**, il **Provider Layer AI (7a)**, lo **StructuredOutputPort (7b)**, l'**AI Master pipeline (7c)**, il **Canon Ledger L1.5 (8a)**, **Reflection + L2 + Salienza (8b)**, il **Context Assembler (8c)** e la **Shell Electron (9a)** sono **finiti e mergiati** in `main`. I Piani 7, **8** e il **9a** sono completi. **Piano 9 splittato in 9a/9b/9c** (vedi §7-bis e il piano `...piano9a-shell-electron.md`): **9a fatto** = pacchetto `app/desktop` (electron-vite + Vue), sicurezza (contextIsolation/sandbox/nodeIntegration:false), contratto IPC tipizzato in `@loomn/shared`, giro IPC main↔renderer + push read-side, **senza DB/AI**. Il **prossimo passo è scrivere ed eseguire il Piano 9b — Wiring memoria+AI**: nuovo pacchetto **`@loomn/host`** (dipende da engine/memory/ai/shared) con `createSqliteEventStoreOn(db)` (event store su handle condiviso — richiede piccola modifica a `@loomn/memory`), `createMemorySystem(dbPath)` (UNA connessione: EventStore+CanonLedger+SummaryStore+ContextAssembler), `systemClock` (porta **`Clock`** reale), impl **LLM-backed** di `FactExtractor`/`Summarizer` (via lo `StructuredOutputPort` di 7b — la composizione `ai`+`memory` avviene **solo qui**), assembler **iniettato** in `runMasterTurn`. Tutto unit-testabile (ABI Node). Poi **9c** = IPC write/read reale + persistenza nell'app (better-sqlite3 ricompilato per ABI Electron + copia migrazioni in `out/migrations` + gestione conflitto ABI Node↔Electron). (Decisione presa in 8c: gli strumenti rimandati di 7c `request_check`/`apply_effect`/`advance_quest` + la **FSM di fase** §5.5 NON sono in 8c → **traccia engine separata**, un piano dedicato.) Tutti i piani sono in `docs/superpowers/plans/`.
...
- **Piano 9 — Shell Electron, splittato in 9a/9b/9c:**
  ...
  - **9b — Wiring memoria+AI (core testabile)** ← *prossimo* — nuovo pacchetto **`@loomn/host`** (dipende da engine/memory/ai/shared): `createSqliteEventStoreOn(db)` (event store su handle — richiede modifica a `@loomn/memory`), `createMemorySystem(dbPath)` (UNA connessione: EventStore+CanonLedger+SummaryStore+ContextAssembler), `systemClock` (porta **`Clock`** reale, per i meta degli eventi e il `created_at` di L2), impl **LLM-backed** di `FactExtractor`/`Summarizer` (su `StructuredOutputPort` di 7b — composizione `ai`+`memory` **solo qui**), assembler iniettato in `runMasterTurn`. Unit-testabile (ABI Node).
  - **9c — IPC write/read reale + persistenza nell'app** — `app/desktop` apre il DB via `@loomn/host` (better-sqlite3 ricompilato per ABI Electron + plugin copia migrazioni in `out/migrations`; gestione conflitto ABI Node↔Electron), canale IPC dei Command (write side: `decide`→persisti→proietta), push degli Event al renderer (read side), `runMasterTurn` dietro IPC con l'assembler iniettato + `safeStorage` per le chiavi provider.
...
1. Leggi questo file (in particolare **§7-bis**: cosa ha consegnato 9a e cosa è già verificato per 9b/9c — NON rifare la sandbox) + lo spec (per il **Piano 9b** contano **§5.4** pipeline AI Master, **§6.1** Reflection, **§6.2** Context Assembler, **§7** strato AI/StructuredOutput; per 9c **§5.2** CQRS, **§6.3** persistenza) + (se utile) i Piani 8c (`createContextAssembler` + iniezione `AssembleContext`), 8b (`runReflection` con porte `FactExtractor`/`Summarizer`/`Clock`), 8a (`createCanonLedger`) e 6 (`createSqliteEventStore`, `openDatabase`).
2. `git status` (atteso pulito, su `main`, HEAD `cbac493`+) e `pnpm test` (atteso **221 verdi**), `pnpm -r typecheck` pulito (incluso `app/desktop` via `vue-tsc`).
3. Quando l'utente dice "scrivi il Piano 9b": usa `writing-plans`, applica le house rules (§5), parti dalla sintesi in §0/§7-bis/§8. **Piano 9b = `@loomn/host`** (nuovo pacchetto `packages/host` che dipende da engine/memory/ai/shared — è l'unico modo di comporre `ai`+`memory`, resta sotto l'attuale `vitest.config.ts`): estendi `@loomn/memory` con `createSqliteEventStoreOn(db)` (event store su handle — prototipo in §7-bis); `createMemorySystem(dbPath)` (UNA connessione: EventStore+CanonLedger+SummaryStore+ContextAssembler); `systemClock` (porta `Clock` reale); impl **LLM-backed** di `FactExtractor`/`Summarizer` (su `createStructuredOutput` di 7b — `ai`+`memory` si compongono **solo qui**); assembler iniettato in `runMasterTurn` al posto di `assembleContextStub`. Tutto unit-testabile a porte iniettate (ABI Node → i test restano verdi; il nativo per Electron è un concern di 9c, vedi §7-bis). NON serve nuova sandbox per 9b: il groundwork è in §7-bis. (Lo split 9a/9b/9c è già deciso; 9a fatto.)
