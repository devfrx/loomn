...
> **Data:** 2026-06-16 · **Branch:** `main` · **HEAD:** `fd533c3` (più eventuali commit doc successivi — fai `git log`) · **Stato:** Piani 1-6 + **7a/7b/7c + 8a/8b/8c** completi e mergiati (engine + persistenza + Provider Layer AI + StructuredOutputPort + AI Master pipeline + Canon Ledger L1.5 + Reflection/L2/Salienza + **Context Assembler**), **215 test verdi**, typecheck pulito, tree pulito.
>
...
L'engine (Piani 1-5), la **Persistenza (6)**, il **Provider Layer AI (7a)**, lo **StructuredOutputPort (7b)**, l'**AI Master pipeline (7c)**, il **Canon Ledger L1.5 (8a)**, **Reflection + L2 + Salienza (8b)** e il **Context Assembler (8c)** sono **finiti e mergiati** in `main`. I Piani 7 e **8 sono completi** (8 = splittato in 8a/8b/8c, tutti fatti). Il prossimo passo è **scrivere ed eseguire il Piano 9 — Shell Electron**: main/preload/renderer con sicurezza (contextIsolation/sandbox/safeStorage), IPC tipizzato, la porta **`Clock`** reale (per i meta degli eventi e il `created_at` di L2), le impl **LLM-backed** di `FactExtractor`/`Summarizer` (via lo `StructuredOutputPort` di 7b — la composizione `ai`+`memory` avviene **solo qui**), e il **wiring** di EventStore + CanonLedger + SummaryStore + **ContextAssembler** su connessione condivisa, con l'assembler **iniettato** in `runMasterTurn` al posto di `assembleContextStub`. (Decisione presa in 8c: gli strumenti rimandati di 7c `request_check`/`apply_effect`/`advance_quest` + la **FSM di fase** §5.5 NON sono in 8c → **traccia engine separata**, un piano dedicato.) Tutti i piani sono in `docs/superpowers/plans/`.
...
- **Piano 9 — Shell Electron** ← *prossimo* (main/preload/renderer, sicurezza contextIsolation/sandbox/safeStorage, IPC tipizzato, **Clock** reale (porta definita in 8b) per i meta degli eventi e il `created_at` di L2; impl LLM-backed di `FactExtractor`/`Summarizer` per la Reflection (usano lo `StructuredOutputPort` di 7b — composizione `ai`+`memory` solo qui nell'app); wiring di EventStore+CanonLedger+SummaryStore+**ContextAssembler** su connessione condivisa, con l'assembler iniettato in `runMasterTurn`)
...
1. Leggi questo file + lo spec (per il Piano 9 — Shell Electron — contano **§5.2** CQRS attraverso i processi Electron, **§6.3** persistenza offline-first, **§7** strato AI/provider) + (se utile) i Piani 8c (`createContextAssembler` + il punto di iniezione `AssembleContext` su `runMasterTurn`), 8b (`runReflection` con porte `FactExtractor`/`Summarizer`/`Clock`), 8a (`createCanonLedger`) e 6 (`createSqliteEventStore`, `openDatabase`).
2. `git status` (atteso pulito, su `main`) e `pnpm test` (atteso **215 verdi**), `pnpm typecheck` pulito.
3. Quando l'utente dice "scrivi il Piano 9": usa `writing-plans`, applica le house rules (§5), parti dalla sintesi in §0/§8. Esiste già tutto il core testabile a unità: `@loomn/memory` (EventStore SQLite + snapshot 6, Canon Ledger 8a, L2/Reflection/Salienza 8b, **Context Assembler 8c** `createContextAssembler`) e `@loomn/ai` (provider layer 7a, StructuredOutputPort 7b, AI Master pipeline 7c — `runMasterTurn` accetta l'iniezione `assembleContext`). Piano 9 = **Shell Electron**: main/preload/renderer con sicurezza (contextIsolation/sandbox/safeStorage), IPC tipizzato (write side nel main, read side = proiezioni di sola lettura nel renderer, spec §5.2), la porta **`Clock`** reale, le impl **LLM-backed** di `FactExtractor`/`Summarizer` (via lo `StructuredOutputPort` di 7b — `ai`+`memory` si compongono **solo qui**), e il **wiring** di EventStore+CanonLedger+SummaryStore+**ContextAssembler** su **una** connessione better-sqlite3 condivisa, con l'assembler iniettato in `runMasterTurn` al posto di `assembleContextStub`. ⚠️ Il nativo better-sqlite3 va ricompilato per l'ABI di Electron (`@electron/rebuild`) — **verifica empiricamente in sandbox**. Da qui si entra nell'IO reale (Electron): i piani saranno meno puramente unit-testabili (§8). Valuta lo split (main+sicurezza / wiring memoria+AI / IPC+renderer).
