...
## Ri-validazione post-G1 (2026-06-16, notte)

Dopo aver mergiato **G1** (schemi tool coercivi-ma-strict, `d1ec4a5`) ho ri-eseguito l'harness su `main` (HEAD con G1) contro `meta-llama-3.1-8b-instruct` (LM Studio), portando i file dal branch `spike/slice-llm`. Esito (osservazioni in `<tmp>/loomn-spike-observations.md`):

- ✅ **G1 validato sul campo.** Al turno 2 il modello ha emesso `"defenseBase": "10"` (stringa) e la coercizione l'ha accettato → `defenseBase: 10` (numero) → Command valido → **il motore ha risolto l'attacco** (`AttackResolved`, d20=1 → miss deterministico, ma l'attacco È stato risolto). Nel run precedente la stessa stringa bloccava l'intero combattimento; ora il combattimento si sblocca. La rete di sicurezza strict ha anche tenuto: campi vuoti (`skill:""`, `defense:""`, …) rifiutati da Zod come prima.
- 🔴 **F4 ri-confermato, forte.** `reflect('scena-1')` → `factCount=3`, e i 3 fatti L1.5 sono **tutti meccanici/derivati dagli Event** ("Eldra aggiunto come giocatore", "Krix aggiunto come NPC", "Eldra ha attaccato goblin_mercante"); il riassunto L2 è una recitazione degli Event ("...Eldra ha attaccato Krix ma non ha colpito"). **Tutta la narrazione è sparita**: l'affare losco di Krix, Krix che serve il **Barone Vhalmar di Pietranera**, la supplica — niente in L1.5/L2. In più, in questo run il Barone non è nemmeno diventato un `ActorAdded` → **assente del tutto** dalla memoria. È esattamente F4: la memoria ricorda le statistiche e dimentica la storia. → **F4 è la prossima mossa giusta.**
- 🟠 **G6 (NUOVO) — il modello stringifica gli argomenti ARRAY (stesso vizio di G1, ma per le strutture).** Al turno 3 `start_encounter` è stato proposto più volte con `participants` come **stringa JSON** (`"participants": "[{...}]"`) invece che array → `Expected array, received string` → lo scontro non è mai partito (turno senza eventi). Lo stesso vizio compare nella Reflection: la strategia function-call di `extract_facts` fallisce con `facts: Expected array, received string` e ripiega su repair (F3/G5). È la versione "array" di G1: candidato a una piccola coercizione gemella (un `z.preprocess` che fa `JSON.parse` se riceve una stringa, poi valida come array) per `participants` (e utile anche al write-path della Reflection). Da valutare con l'utente come "G1b" nella traccia engine/AI.
- 🟢 **"Il codice è l'arbitro" REGGE ancora:** ogni argomento invalido rifiutato da Zod; il motore ha rifiutato `next_round` (nessuno scontro attivo) e l'attacco con `damageResource:"danno"` (risorsa sconosciuta, G3) quando il tiro era un colpo; zero mutazioni dirette. G3/G4 ri-confermati (identificatori inventati: `skill:"spada corta"`, `defense:"difesa corpo a corpo"`, `damageResource:"danno"`; Krix nato con `resources:{denaro}`, senza `hp`). F2 non esercitato (template tool sano → nessun frame SSE di errore). Collisione id sulla seconda `reflect` ri-prodotta.

## Note di metodo

Lo spike (harness + test guardato) vive sul branch `spike/slice-llm`, è **throwaway**: non tocca manifest/config né la suite (`pnpm test` resta **verde**, lo spike è inerte senza `LOOMN_SPIKE=1`). Su `main` va **solo questo documento**. La ri-validazione post-G1 ha riportato i due file dal branch nel working tree, eseguito, e li ha rimossi (main resta pulito).
