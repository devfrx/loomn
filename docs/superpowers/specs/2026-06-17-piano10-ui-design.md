...
| Legalità comandi per fase (disabilitare i bottoni GM illegali) | `isCommandLegalInPhase` (engine) | 🟡 read-layer → **10g** |
| Config provider corrente (pre-compilare Impostazioni: baseUrl/model, mai la chiave) | `settings.ts`/`loadProviderConfig` (main) | 🟡 read-layer → **10f** |
...
**Rischi d'integrazione da validare (pre-flag, non sono lacune backend):**
- **Dadi/font/CSP (spike di 10b; font in 10a):** `@3d-dice/dice-box` usa **wasm + (tipicamente) un web worker** e carica asset a runtime; i font letterari (Newsreader/Archivo/JetBrains Mono/Fraunces) vanno **bundlati offline**. Con `sandbox:true` + la CSP del renderer (dal 9a) questo può bloccare wasm/worker/font → lo spike deve validare il **bundling degli asset** (pattern della copia `migrations`→`out` del 9c-ii) ed eventualmente adeguare CSP/`electron.vite.config` (passo orchestratore).
- **Self-test scriptabile (10a):** oggi `LOOMN_SELFTEST` vive in `renderer.ts` come flusso non-GUI; quando il renderer diventa la vera app Vue va mantenuto funzionante (skip GUI / drive dei nuovi flussi) → pianificarne esplicitamente l'evoluzione in 10a (il gate "esegui l'app" deve continuare a dare `VERDICT: PASS`).
...
| **10g · Metadati di gioco su IPC** (read channel `get-ruleset`, stile Piano 0) | canale read `get-ruleset` → DTO `{vocabulary:{attributes,skills,resources,defenses,defaultResources}, difficulties, softPhases, questOutcomes, directions, commandPhaseRules:{combatOnly,nonCombatOnly}}`; metodo sincrono su `CampaignService` (come i read del Piano 0) + handler IPC sottile + bridge; export degli enum statici di comando da `@loomn/shared`; self-test esteso. Include le **regole di legalità-per-fase** (da `isCommandLegalInPhase`) per disabilitare i comandi GM illegali nella fase corrente. Additivo, testabile su ABI Node, `shared` resta foglia | `Ruleset` (host), enum engine, `isCommandLegalInPhase` | 0 |
| **10f · Impostazioni + first-run + GM** | provider (set-provider/safeStorage/get-status), **read-back config provider** (baseUrl/model via `get-status` esteso o `get-provider`, mai la chiave), first-run, **creazione PG** (AddActor), controlli GM/manuali (Command estesi) — **form data-driven dal vocabolario+regole di 10g** | `set-provider`/`get-status` (esteso), `commandSchema` esteso, `get-ruleset` | 0, 10a, 10g |
...
| **10c · Combattimento** | cockpit scontro (iniziativa/round/turni), **zone display-only** (label/raggruppamento da `encounter.participants[].zone`; **NO** mappa spaziale / azione muovi → deferito, vedi §11), feedback attacco/check/effetto (`RollResult` negli `events`), re-theme per fase | `GameState.encounter`, Command combat | 10a, 10b |
...
- **Equip/movimento come azioni** (Command/Event): feature future; per ora display-only. In particolare il **movimento in zona** (topologia/mappa di zona persistita + `Command`/`Event` di spostamento) è una **traccia engine deferita post-Piano-10**, co-progettata con l'Inventario: l'engine ha già la matematica (`zone.ts` = `areAdjacent`/`zoneDistance`/`rangeBand`, `encounter.ts` = `moveParticipant`) ma **non è cablata a `Command`/`Event`** e **nessuna topologia di zona vive in `GameState`/`Ruleset`** (l'`Encounter` porta solo `participants[].zone` come stringa). → **10c rende le zone display-only** (label/raggruppamento), niente mappa spaziale né azione muovi. *(Decisione bloccata con l'utente 2026-06-18: tenere il movimento deferito per non deviare l'engine a metà-UI; si costruirà come traccia engine stile SP1-4 dopo il Piano 10, insieme all'Inventario.)*
